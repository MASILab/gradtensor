...
cpost = '/emp_corrected_rd.nii'
for i in range(1,20):
    outdir = cpre + 'OUTPUT_masivar_SNR10_' +str(i) + cpost
...
true_fa = nib.load('/home/local/VANDERBILT/kanakap/gradtensor_data/10_29_2019_human_repositioned/3tb/posA/OUTPUTS_masiver/true__rd.nii').get_fdata()
true_fa_dict = {}
for i in range(1,20):
...
img = nib.load('/home/local/VANDERBILT/kanakap/gradtensor_data/10_29_2019_human_repositioned/3tb/posA/OUTPUTS_masiver/true__rd.nii')
coh = nib.Nifti1Image(aa, img.affine, img.header)
nib.save(coh, 'cohend_image_snr10_emp_rd.nii')
